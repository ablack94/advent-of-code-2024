...
def concat(x, y):
    return (x * 10 ** (math.floor(math.log(y) / math.log(10)) + 1)) + y
...
def is_solvable(goal, cur, remaining):
    ...
    return (
        is_solvable(goal, cur + next_value, next_remaining)
        or is_solvable(goal, cur * next_value, next_remaining)
        or is_solvable(goal, concat(cur, next_value), next_remaining)
    )
